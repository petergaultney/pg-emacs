...
(defun my-get-error-log-path ()
  ...
  (let* ((log-dir "~/.emacs.d/error-logs-by-date")
...
(defun my-ignore-window-resize-errors (error-data)
  ...
  (condition-case err
    ...
    (error
      (message "Error while checking backtrace: %S" err)
      nil)))
    ...
    (unwind-protect
      (progn
        (condition-case err
          ...
          (error
            (message "Error in logger itself: %S" err))))
